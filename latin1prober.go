...
func NewLatin1Prober() CharsetProber {
	l := &Latin1Prober{
...
func (l *Latin1Prober) charName() string {
	return "ISO-8859-1"
}
...
func (l *Latin1Prober) feed(data []byte) ProbingState {
	...
	for _, c := range buf {
		charClass := Latin1_CharToClass[int(c)]
		freq := Latin1ClassModel[l.lastCharClass*CLASS_NUM+charClass]
		if freq == 0 {
			l.state_ = NOT_ME
			break
		}
